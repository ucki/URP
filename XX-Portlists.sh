...
for word in $(cat ./results/online-IP.txt); do cat ./results/$word/$word-nmap*.xml |grep portid |grep open|cut -d ">" -f 1 | cut -d "=" -f 2,3 |cut -d "\"" -f 2,4 |grep tcp|cut -d "\"" -f 2 |sort -u > ./results/$word/$word-TCPPortlist.txt & done
...
for word in $(cat ./results/online-IP.txt); do cat ./results/$word/$word-nmap*.xml |grep portid |grep open|cut -d ">" -f 1 | cut -d "=" -f 2,3 |cut -d "\"" -f 2,4 |grep udp|cut -d "\"" -f 2 |sort -u > ./results/$word/$word-UDPPortlist.txt & done
